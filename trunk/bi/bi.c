...
////////////////////////////////////
// ∞—main.exe÷–rela.text∫Õrela.data∑≈‘⁄“ª∏ˆµ•∂¿µƒsection÷–
...
/////////////////////////////////////////////for rela.text////////////////////////////////////////////
	for (i=0; i<rela_size; i+=sizeof(rela))
	{
		...
		else if (symbol.st_shndx == SHN_ABS) addr = symbol.st_value; // æ¯∂‘µÿ÷∑
		//else if (symbol.st_shndx == SHN_COMMON) {} // the section which the symbol resides in not allocated yet!!!
		//else if (symbol.st_shndx == SHN_UNDEF) {}
		
		if (rela_sec == textndx) offset = text_off - text_addr; // off: Œƒº˛∆´“∆, addr: memory address
		...
		// ø…“‘Õ®π˝realbΩ¯––∑¥œÚ≤È’“£¨’“µΩœ‡”¶µƒsymbol
		...
		// (2) write to rel
		// –Ë“™–ﬁ’˝µƒµÿ÷∑ address in the file or the virtual address (i.e. load address)
		// we use the virtual address first
		myloc.r_offset = rela.r_offset; // ‘⁄ƒƒ–Ë“™–ﬁ∏ƒ
		
		lseek(in, offset, SEEK_SET);
		read(in, &myloc.r_addr, 2); // –ﬁ∏ƒ≥… ≤√¥÷µ
