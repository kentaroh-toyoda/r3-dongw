...
module CDataC 
{
	uses 
	{
		...
		interface Receive as EventReceiver;
...
implementation 
{
	...
	{
		if (call SerialSender.send(AM_BROADCAST_ADDR, &uartbuf, uartlen) != SUCCESS)
		{
			report_problem();
		} 
		else 
	...
	{
		uint32_t localtime = INVALID_TIME;
		call GlobalTime.getGlobalTime(&localtime);
		pr("global time:%ld, len:%d\n, payload:%d, sizeof:%d", localtime, len, *(uint8_t*)payload, sizeof(cstatus_t));
		
		
		uartlen = len + sizeof(ctp_data_header_t);
		//set the recv time
		*((uint8_t*)(payload + len + 0)) = *((uint8_t*)(&localtime) + 3);
		*((uint8_t*)(payload + len + 1)) = *((uint8_t*)(&localtime) + 2);
		*((uint8_t*)(payload + len + 2)) = *((uint8_t*)(&localtime) + 1);
		*((uint8_t*)(payload + len + 3)) = *((uint8_t*)(&localtime) + 0);

//		*((uint32_t*)(payload + len)) = localtime;
//		memcpy(payload + len, &localtime, sizeof(uint32_t));
		uartlen += sizeof(uint32_t);
		if (uartbusy == FALSE) 
		{
	...
	event message_t* EventReceiver.receive(message_t* msg, void* payload, uint8_t len) 
	{
		uint32_t localtime = INVALID_TIME;
		call GlobalTime.getGlobalTime(&localtime);
		pr("global time:%ld, len:%d\n, payload:%d, sizeof:%d", localtime, len, *(uint8_t*)payload, sizeof(cstatus_t));
